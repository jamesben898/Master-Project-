...
1. Prepare DEG List
Since you already have the DESeq2 results (res), we'll extract DEGs with filtering criteria:
•	log2 Fold Change (LFC) > 1 or < -1 (for significant change)
•	Adjusted p-value (padj) < 0.05 (to control for false positives)
Step 1: Filter the Results Based on Criteria
# Filter DEGs based on log2 fold change and adjusted p-value
deg_list <- res[abs(res$log2FoldChange) > 1 & res$padj < 0.05, ]

# Optionally, sort by adjusted p-value or log2 fold change for top-ranked genes
deg_list_sorted <- deg_list[order(deg_list$padj), ]

# View the top DEGs
head(deg_list_sorted)

# Save the DEG list to a .csv file
write.csv(deg_list_sorted, "/path/to/your/external/drive/deg_list.csv", row.names = TRUE)
...
> 
> # Filter DEGs based on log2 fold change and adjusted p-value
> deg_list <- res_clean[abs(res_clean$log2FoldChange) > 1 & res_clean$padj < 0.05, ]
> 
> # Optionally, sort by adjusted p-value or log2 fold change for top-ranked genes
> deg_list_sorted <- deg_list[order(deg_list$padj), ]
> 
...
2. Gene Annotation
To annotate your DEGs, you can use the biomaRt package to convert Ensembl Gene IDs to gene symbols or KEGG IDs.

# Remove the version number from Ensembl gene IDs
> short_gene_ids <- gsub("\\..*", "", rownames(deg_list_sorted))
> 
> # Query without version number
> gene_annotations_short <- getBM(attributes = c("ensembl_gene_id", "hgnc_symbol"),
+                                 filters = "ensembl_gene_id",
+                                 values = short_gene_ids,
+                                 mart = mart)
>                                                                      
> # View the result
> head(gene_annotations_short)

Join the Gene Annotations with Your DEG List:

Convert DESeqResults to DataFrame: Convert your DESeqResults object (deg_list_sorted) into a regular data.frame so it can be merged with the gene annotations.

# Convert the DESeqResults object to a data.frame
deg_list_sorted_df <- as.data.frame(deg_list_sorted)

# Merge the DEG list with gene annotations
deg_list_sorted_annotated <- merge(deg_list_sorted_df, gene_annotations_short, by.x = "row.names", by.y = "ensembl_gene_id", all.x = TRUE)

# View the merged result
head(deg_list_sorted_annotated)

Join the Gene Annotations with Your DEG List:

library(org.Hs.eg.db)
> 
> # Strip version numbers from Ensembl gene IDs (e.g., ENSG00000000003.16 -> ENSG00000000003)
> gene_ids_no_version <- sub("\\.\\d+$", "", missing_hgnc$Row.names)
> 
> # Query for HGNC symbols using the stripped Ensembl gene IDs
> hgnc_from_orgdb <- mapIds(org.Hs.eg.db, 
+                           keys = gene_ids_no_version,    # Ensembl gene IDs without version
+                           column = "SYMBOL",              # HGNC symbols
+                           keytype = "ENSEMBL",            # Specify Ensembl as the key type
+                           multiVals = "first")            # Choose the first match if multiple exist
'select()' returned 1:many mapping between keys and columns
> 
> # View the result
> head(hgnc_from_orgdb)
ENSG00000000003 ENSG00000000005 ENSG00000000419 ENSG00000000457 ENSG00000000460 
       "TSPAN6"          "TNMD"          "DPM1"         "SCYL3"         "FIRRM" 
ENSG00000000938 
          "FGR" 

# Strip version numbers from Ensembl gene IDs in deg_list_sorted
> deg_list_sorted$ensembl_gene_id <- sub("\\.\\d+$", "", rownames(deg_list_sorted))
> 
> # Query for HGNC symbols using the stripped Ensembl gene IDs
> hgnc_from_orgdb <- mapIds(org.Hs.eg.db, 
+                           keys = deg_list_sorted$ensembl_gene_id,    # Ensembl gene IDs without version
+                           column = "SYMBOL",                         # HGNC symbols
+                           keytype = "ENSEMBL",                       # Specify Ensembl as the key type
+                           multiVals = "first")                       # Choose the first match if multiple exist
'select()' returned 1:many mapping between keys and columns
> 
> # Add the HGNC symbols to the DEG list
> deg_list_sorted$hgnc_symbol <- hgnc_from_orgdb
> 
> # View the updated DEG list
> head(deg_list_sorted)
log2 fold change (MLE): Intercept 
Wald test p-value: Intercept 
DataFrame with 6 rows and 8 columns
                    baseMean log2FoldChange     lfcSE      stat    pvalue
                   <numeric>      <numeric> <numeric> <numeric> <numeric>
ENSG00000000003.16   217.517        7.76498 0.0556106   139.631         0
ENSG00000000419.14   428.121        8.74187 0.0237546   368.008         0
ENSG00000000457.14   259.791        8.02121 0.0297741   269.402         0
ENSG00000000460.17   108.509        6.76167 0.0490238   137.926         0
ENSG00000000938.13   133.452        7.06018 0.0640678   110.198         0
ENSG00000000971.17   630.397        9.30012 0.0602246   154.424         0
                        padj ensembl_gene_id hgnc_symbol
                   <numeric>     <character> <character>
ENSG00000000003.16         0 ENSG00000000003      TSPAN6
ENSG00000000419.14         0 ENSG00000000419        DPM1
ENSG00000000457.14         0 ENSG00000000457       SCYL3
ENSG00000000460.17         0 ENSG00000000460       FIRRM
ENSG00000000938.13         0 ENSG00000000938         FGR
ENSG00000000971.17         0 ENSG00000000971         CFH
> 
> # Save the updated DEG list to a .csv file
> write.csv(deg_list_sorted, "/Volumes/One Touch/masters project stuff/deg_list_with_hgnc.csv", row.names = TRUE)
...
+                      keys = deg_list_sorted$hgnc_symbol, 
+                      column = "ENTREZID", 
+                      keytype = "SYMBOL", 
+                      multiVals = "first")
'select()' returned 1:many mapping between keys and columns
> 
> # Add Entrez IDs to the DEG list
> deg_list_sorted$entrezgene_id <- entrez_ids
> 
> # Check the updated DEG list with Entrez IDs
> head(deg_list_sorted)


# Install and load clusterProfiler if not already done
> if (!requireNamespace("clusterProfiler", quietly = TRUE)) {
+     BiocManager::install("clusterProfiler")
+ }
> library(clusterProfiler)
> 
> # Perform KEGG enrichment analysis with Entrez gene IDs
> kegg_enrichment <- enrichKEGG(gene = deg_list_sorted$entrezgene_id,
+                               organism = 'hsa', 
+                               pvalueCutoff = 0.05)
> 
> # View results
...
# Save the results
> write.csv(kegg_enrichment, "/Volumes/One Touch/masters project stuff/kegg_enrichment_results.csv", row.names = FALSE)
> 
> # Visualize the top 10 KEGG pathways
> library(enrichplot) barplot(kegg_enrichment, showCategory = 10)

library(ReactomePA)


# Ensure org.Hs.eg.db is loaded
> library(org.Hs.eg.db)
> 
> # Map HGNC symbols to Entrez gene IDs
> entrez_ids <- mapIds(org.Hs.eg.db, 
+                      keys = deg_list_sorted$hgnc_symbol,  # Gene symbols
+                      column = "ENTREZID",  # Target Entrez Gene ID column
+                      keytype = "SYMBOL",   # Use SYMBOL as the key
+                      multiVals = "first")  # If multiple Entrez IDs, take the first
'select()' returned 1:many mapping between keys and columns
> 
> # Check the results
> head(entrez_ids)
...
# Perform Reactome enrichment analysis with Entrez gene IDs
...
> write.csv(reactome_enrichment, "/Volumes/One Touch/masters project stuff/reactome_enrichment_results.csv", row.names = FALSE)

# Annotating the DEG list with Entrez Gene IDs
> deg_list_sorted$entrezgene_id <- entrez_ids  # Add Entrez Gene IDs to the DEG list
> 
> # Check the first few rows of the annotated DEG list
> head(deg_list_sorted)

# Prepare the gene list for DAVID (Entrez Gene IDs)
> david_gene_list <- deg_list_sorted$entrezgene_id
> 
> # Check the gene list
> head(david_gene_list)

# Check if Entrez Gene IDs are present in deg_list_sorted
> head(deg_list_sorted)

# If the column isn't present or has NA values, check the Entrez mapping
> # Add the Entrez Gene IDs (if necessary, re-run the Entrez mapping step)
> deg_list_sorted$entrezgene_id <- entrez_ids  # Ensure this column contains the Entrez Gene IDs
> 
> # Verify the content
> head(deg_list_sorted$entrezgene_id)

# Check the first few row names
> head(rownames(deg_list_sorted))

# Strip version numbers from Ensembl gene IDs (e.g., ENSG00000000003.16 -> ENSG00000000003)
> deg_list_sorted$Row.names <- sub("\\.\\d+$", "", rownames(deg_list_sorted))
> 
> # Check the first few row names after stripping
> head(deg_list_sorted$Row.names)

# Use hgnc_from_orgdb to map Ensembl gene IDs to Entrez Gene IDs
> deg_list_sorted$entrezgene_id <- hgnc_from_orgdb[deg_list_sorted$Row.names]
> 
...
> david_gene_list <- deg_list_sorted$entrezgene_id
> 
> # Save the gene list as a text file for DAVID
> write.table(david_gene_list, "/Volumes/One Touch/masters project stuff/david_gene_list.txt", 
+             row.names = FALSE, col.names = FALSE, quote = FA
